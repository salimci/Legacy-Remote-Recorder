...
namespace Parser
{
    public class LinuxTBursAccessV_1_0_0Recorder : Parser
    {

        private string dateFormat = "yyyy-MM-dd HH:mm:ss";

        public LinuxTBursAccessV_1_0_0Recorder()
            : base()
        {
            LogName = "LinuxTBursAccessV_1_0_0Recorder";
            usingKeywords = false;
            lineLimit = 50;
        }

        public LinuxTBursAccessV_1_0_0Recorder(String fileName)
            : base(fileName)
        {
        ...
        public override bool ParseSpecific(string line, bool dontSend)
        {
            ...
            if (!dontSend)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        string dateString = Before(arr[3], "+").Replace('[', ' ').Trim();
                        string date = Before(dateString, ":");
                        string time = After(dateString, ":", 1);
                    ...
                    r.EventCategory = arr[4].Replace('"', ' ').Trim();
                    ...
                    r.EventType = arr[6].Replace('"', ' ').Trim();
                    ...
                    {
                        Log.Log(LogType.FILE, LogLevel.ERROR, "CustomIn1 TypeCasting Error: " + exception.Message);
                    }

                    try
                    {
                        r.CustomInt2 = Convert.ToInt32(arr[8]);
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "CustomInt2: " + r.CustomInt2);
                    }
                    catch (Exception exception)
                    {
                        Log.Log(LogType.FILE, LogLevel.ERROR, "CustomIn2 TypeCasting Error: " + exception.Message);
                    }
