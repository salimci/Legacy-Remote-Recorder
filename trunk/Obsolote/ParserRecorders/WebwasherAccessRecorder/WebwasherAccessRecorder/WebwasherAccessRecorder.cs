...
namespace Parser
{
    public class WebwasherAccessRecorder : Parser
    {

        public WebwasherAccessRecorder()
            : base()
        {
        ...
        protected override void ParseFileNameRemote()
        {
            ...
            try
            {
                ...
                if (Dir.EndsWith("/") || Dir.EndsWith("\\"))
                {
                    ...
                    {
                        String[] arr = line.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                        if (arr[arr.Length - 1].StartsWith("access") == true && arr[arr.Length - 1].Split(new char[] { '.' }, StringSplitOptions.RemoveEmptyEntries).Length == 2)//Name changed
                        {
        ...
        private string[] SortFiles(ArrayList arrFileNames)
        {
            ...
            try
            {
                ...
                Log.Log(LogType.FILE, LogLevel.INFORM, "SortFiles() -->> Sýralanmýþ dosya isimleri yazýlýyor.");
