...
namespace Parser
{
    public class WebwasherAccessDeniedRecorder : Parser
    {
        public WebwasherAccessDeniedRecorder()
            : base()
        {
            ...
            try
            {
                ...
                if (Dir.EndsWith("/") || Dir.EndsWith("\\"))
                {
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        if (arr[arr.Length - 1].StartsWith("access_") == true && arr[arr.Length - 1].Split(new char[] { '.' }, StringSplitOptions.RemoveEmptyEntries).Length == 2)//Name changed
                        {
        ...
        private string[] SortFiles(ArrayList arrFileNames)
        {
            ...
            try
            {
                for (int i = 0; i < arrFileNames.Count; i++)
                {
                    ...
                    if (parts.Length == 2 && parts[0].Length != 13)
                    {
                        //access_denied1012282359.merged
