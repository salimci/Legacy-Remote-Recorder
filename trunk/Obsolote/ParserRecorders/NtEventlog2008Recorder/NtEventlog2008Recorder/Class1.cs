...
namespace NtEventlog2008Recorder
{
    public class NtEventlog2008Recorder : CustomBase
    {
        ...
        protected object regSync = new object();
            ...
            lock (regSync)
            {
                if (!reg_flag)
                {
                    ...
                    try
                    {
                        ...
                        rk =
                            Registry.LocalMachine.OpenSubKey("SOFTWARE").OpenSubKey("Natek").OpenSubKey(
                                "Security Manager");
                        log_size =
                            Convert.ToUInt32(
                                rk.OpenSubKey("Recorder").OpenSubKey("NtEventlog2008Recorder").GetValue("Log Size"));
                        logging_interval =
                            Convert.ToUInt32(
                                rk.OpenSubKey("Recorder").OpenSubKey("NtEventlog2008Recorder").GetValue(
                                    "Logging Interval"));
                        trc_level =
                            Convert.ToInt32(
                                rk.OpenSubKey("Recorder").OpenSubKey("NtEventlog2008Recorder").GetValue("Trace Level"));
                        remote_host =
                            rk.OpenSubKey("Recorder").OpenSubKey("NtEventlog2008Recorder").GetValue("Remote Host").
                                ToString();
                        err_log = rk.OpenSubKey("Agent").GetValue("Home Directory").ToString() +
                                  @"log\NtEventlog2008Recorder" + remote_host + ".log";
                        this.timer1.Interval =
                            Convert.ToInt32(
                                rk.OpenSubKey("Recorder").OpenSubKey("NtEventlog2008Recorder").GetValue("SleepTime"));
                        max_line_towait =
                            Convert.ToInt32(
                                rk.OpenSubKey("Recorder").OpenSubKey("NtEventlog2008Recorder").GetValue("MaxLineToWait"));
                        fromend =
                            Convert.ToInt32(
                                rk.OpenSubKey("Recorder").OpenSubKey("NtEventlog2008Recorder").GetValue("FromEndOnLoss"));
                        last_position =
                            rk.OpenSubKey("Recorder").OpenSubKey("NtEventlog2008Recorder").GetValue("LastPosition").
                                ToString();
                        location =
                            rk.OpenSubKey("Recorder").OpenSubKey("NtEventlog2008Recorder").GetValue("Location").ToString
                                ();
                        user =
                            rk.OpenSubKey("Recorder").OpenSubKey("NtEventlog2008Recorder").GetValue("User").ToString();
                        password =
                            rk.OpenSubKey("Recorder").OpenSubKey("NtEventlog2008Recorder").GetValue("Password").ToString
                                ();
                        if (password != "")
                    ...
                    {
                        EventLog.WriteEntry("Security Manager NtEventlog 2008 Recorder Read Registry", er.ToString(),
                ...
                if (!reg_flag)
                {
                    ...
                    try
                    {
                        ...
                        err_log = rk.OpenSubKey("Remote Recorder").GetValue("Home Directory").ToString() +
                                  @"log\NtEventlog2008Recorder" + Id + ".log";
                        reg_flag = true;
                    }
                    catch (Exception er)
                    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    L.Log(LogType.FILE, LogLevel.INFORM,
                          "Time passed to retrieve " + moList.Count + " records through WMI: ");
                        ...
                        else
                        {
                            if (!Send_Record(mo))
                            {
                                L.Log(LogType.FILE, LogLevel.DEBUG,
                                      "Error on sending record with record number" + mo["RecordNumber"].ToString());
                            }
                            if (Convert.ToInt64(last_position) < Convert.ToInt64(mo["RecordNumber"]))
                            {
                                ...
                                last_recdate = dat.Substring(0, 4) + "/" + dat.Substring(4, 2) + "/" +
                                               dat.Substring(6, 2) + " " + dat.Substring(8, 2) + ":" +
                                               dat.Substring(10, 2) + ":" + dat.Substring(12, 2);
                                //+ "." + dat.Substring(14, 2);                                 
                                last_recdate =
                                    Convert.ToDateTime(last_recdate).AddMinutes(zone).ToString("yyyy/MM/dd HH:mm:ss");
                            }
                        }
                    }
                    ...
                    if (resetposition)
                    {
                        L.Log(LogType.FILE, LogLevel.WARN,
                              "resetposition:No data come Start Reseting the position of the log files");
                        //Set_LastPosition();
        ...
        protected object logSync = new object();
        ...
        public bool Initialize_Logger()
        {
            lock (logSync)
            {
                if (L == null)
                {
                    ...
                    {
                        L = null;
                        EventLog.WriteEntry("Security Manager NtEventlog 2008 Recorder", er.ToString(),
                                            EventLogEntryType.Error);
                        return false;
                    }
                }
