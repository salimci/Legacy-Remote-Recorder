...
namespace JuniperSyslogV6_0_0Recorder
{
    public class JuniperSyslogV6_0_0Recorder : CustomBase
    {
        ...
        public override void Init()
        {
            ...
            if (usingRegistry)
            {
                if (!Read_Registry())
                {
                    EventLog.WriteEntry("Security Manager JuniperSyslogV6_0_0Recorder Read Registry", "JuniperSyslogV6_0_0Recorder may not working properly ", EventLogEntryType.Error);
                    return;
                }
            }
            else
        ...
        public void get_logDir()
        {
            ...
            try
            {
                ...
                err_log = rk.OpenSubKey("Remote Recorder").GetValue("Home Directory").ToString() + @"log\JuniperSyslogV6_0_0Recorder" + identity + ".log";
            }
            catch (Exception ess)
            {
        ...
        public override void Start()
        {
            try
            {
                ...
                L.Log(LogType.FILE, LogLevel.INFORM, "Finish initializing  JuniperSyslogV6_0_0Recorder");
            ...
            {
                EventLog.WriteEntry("Security Manager JuniperSyslogV6_0_0Recorder Constructor", er.ToString(), EventLogEntryType.Error);
            }

        }

        public JuniperSyslogV6_0_0Recorder()
        {
        ...
        void Juniper_SyslogV6_0_0(LogMgrEventArgs args)
        {
            ...
            CustomServiceBase s;
            r.LogName = "Juniper Syslog V6.0.0 Recorder";
            L.Log(LogType.FILE, LogLevel.INFORM, "Line: " + args.Message);
            if (usingRegistry)
            {
            ...
            try
            {
                ...
                else
                {
                    if (arr[arr.Length - 2].Contains("(") & arr[arr.Length - 1].Contains(")"))
                    {
                        ...
                        for (; i < arr.Length - 1; i++)
                        {
                            ...
                            //proto ve Occurred arasýndakileri okumak icin
                            if (bKontrol == true)
                ...
                string description = args.Message;
                if (description.Length > 899)
                {
                    r.Description = description.Substring(0, 899);
                }
        ...
        public bool Read_Registry()
        {
            ...
            {
                rk = Registry.LocalMachine.OpenSubKey("SOFTWARE").OpenSubKey("Natek").OpenSubKey("Security Manager");
                err_log = rk.OpenSubKey("Agent").GetValue("Home Directory").ToString() + @"log\JuniperSyslogV6_0_0Recorder.log";
                Syslog_Port = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("JuniperSyslogV6_0_0Recorder").GetValue("Syslog Port"));
                trc_level = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("JuniperSyslogV6_0_0Recorder").GetValue("Trace Level"));
                protocol = rk.OpenSubKey("Recorder").OpenSubKey("JuniperSyslogV6_0_0Recorder").GetValue("Protocol").ToString();
                return true;
            }
            catch (Exception er)
            {
                EventLog.WriteEntry("Security Manager JuniperSyslogV6_0_0Recorder Read Registry", er.ToString(), EventLogEntryType.Error);
                return false;
            }
            finally
            {
        ...
        public bool Initialize_Logger()
        {
            ...
            {
                EventLog.WriteEntry("Security Manager JuniperSyslogV6_0_0Recorder Recorder", er.ToString(), EventLogEntryType.Error);
                return false;
            }
        }
