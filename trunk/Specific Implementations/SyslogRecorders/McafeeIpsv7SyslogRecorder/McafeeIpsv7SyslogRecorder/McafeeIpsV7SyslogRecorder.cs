...
namespace McafeeIpsV7SyslogRecorder
{
    public class McafeeIpsV7SyslogRecorder : CustomBase
    {
        ...
        public McafeeIpsV7SyslogRecorder()
        {
        ...
        public override void Init()
        {
            try
            {
                ...
                _log.Log(LogType.FILE, LogLevel.INFORM, "Init() -->> Finish initializing McafeeIpsV7SyslogRecorder Event");

            }
            catch (Exception er)
            {
                EventLog.WriteEntry("Security Manager McafeeIpsV7SyslogRecorder Init", er.ToString(), EventLogEntryType.Error);
            }
        }
        ...
        private Boolean InitializeLogger()
        {
            ...
            {
                EventLog.WriteEntry("Security Manager McafeeIpsV7SyslogRecorder", er.ToString(), EventLogEntryType.Error);
                return false;
            }
        }
        ...
        private Boolean GetLogDir()
        {
            ...
            try
            {
                ...
                _errLog = rk.GetValue("Home Directory").ToString() + @"log\" + "McafeeIpsV7SyslogRecorder" + _id + ".log";
                rk.Close();
                return true;
            }
            catch (Exception ex)
            {
                EventLog.WriteEntry("Security Manager McafeeIpsV7SyslogRecorder Read Registry", ex.ToString(), EventLogEntryType.Error);
                return false;
            }
            finally
            {
        ...
        void SlogSyslogEvent(LogMgrEventArgs args)
        {
            try
            {
                ...
                rec.LogName = "McafeeIpsV7SyslogRecorder";
