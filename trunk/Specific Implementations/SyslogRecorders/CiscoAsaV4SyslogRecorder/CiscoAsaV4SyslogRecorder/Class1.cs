...
namespace CiscoAsaV4SyslogRecorder
{
    public class CiscoAsaV4SyslogRecorder : CustomBase
    {
        ...
        public override void Init()
        {
            try
            {
                if (usingRegistry)
                {
                    if (!reg_flag)
                    {
                        if (!Read_Registry())
                        {
                            ...
                            {
                                L.Log(LogType.FILE, LogLevel.ERROR, "Error on Intialize Logger on CiscoAsaSyslogV4 Recorder functions may not be running");
                                return;
                            }
                        reg_flag = true;
                    }
                ...
                L.Log(LogType.FILE, LogLevel.INFORM, "Finish initializing CiscoAsaSyslogV4 Event");
            }
            catch (Exception er)
            {
                EventLog.WriteEntry("Security Manager CiscoAsaSyslogV4 Recorder Init", er.ToString(), EventLogEntryType.Error);
            }
        }
        public bool Get_logDir()
        {
            ...
            try
            {
                ...
                err_log = rk.OpenSubKey("Remote Recorder").GetValue("Home Directory").ToString() + @"log\CiscoAsaV4SyslogRecorder" + Id + ".log";
                rk.Close();
                return true;
            }
            catch (Exception er)
            {
                EventLog.WriteEntry("Security Manager CiscoAsaSyslogV4 Recorder Read Registry", er.ToString(), EventLogEntryType.Error);
                return false;
            }
            finally
            {
        ...
        public CiscoAsaV4SyslogRecorder()
        {
        }
        ...
        void slog_SyslogEvent(LogMgrEventArgs args)
        {
            ...
            try
            {
                ...
                rec.LogName = "CiscoAsaSyslogV4 Recorder";
                ...
                switch (type)
                {
                    ...
                    case "313004":
                        {
                            rec.EventId = 313004;
                            string[] s = args.Message.Split(' ');
                            for (int i = 0; i < s.Length; i++)
                            {
                                if (s[i] == "laddr")
                                {
                                    rec.CustomStr3 = s[i + 1];
                                }
                                if (s[i] == "to")
                                {
                                    rec.CustomStr4 = s[i + 1];
                                    if (rec.CustomStr4.EndsWith(":"))
                                    {
                                        rec.CustomStr4 = rec.CustomStr4.Replace(":", " ").Trim();

                                    }
                                }
                                if (s[i].Contains("313004"))
                                {
                                    rec.EventCategory = s[i + 1];
                                }
                            }
                            rec.CustomStr7 = Between(args.Message, "on", "to");
                            L.Log(LogType.FILE, LogLevel.INFORM, "Onur :  Log : " + args.Message);
        ...
        public bool Initialize_Logger()
        {
            ...
            {
                EventLog.WriteEntry("Security Manager CiscoAsaSyslogV4 Recorder", er.ToString(), EventLogEntryType.Error);
                return false;
            }
        }
