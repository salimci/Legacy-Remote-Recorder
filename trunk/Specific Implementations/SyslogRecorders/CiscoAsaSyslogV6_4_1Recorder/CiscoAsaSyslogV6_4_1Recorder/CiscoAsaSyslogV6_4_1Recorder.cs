...
namespace CiscoAsaSyslogV6_4_1Recorder
{
    public class CiscoAsaSyslogV6_4_1Recorder : CustomBase
    {
        ...
        public override void Init()
        {
            try
            {
                if (usingRegistry)
                {
                    if (!reg_flag)
                    {
                        if (!Read_Registry())
                        {
                            ...
                            {
                                L.Log(LogType.FILE, LogLevel.ERROR, "Error on Intialize Logger on CiscoAsaV6_4_1Recorder Recorder functions may not be running");
                                return;
                            }
                        reg_flag = true;
                    }
                ...
                L.Log(LogType.FILE, LogLevel.INFORM, "Finish initializing CiscoAsaV6_4_1Recorder Event");
            }
            catch (Exception er)
            {
                EventLog.WriteEntry("Security Manager CiscoAsaV6_4_1Recorder Recorder Init", er.ToString(), EventLogEntryType.Error);
            }
        }
        public bool Get_logDir()
        {
            ...
            try
            {
                ...
                err_log = rk.OpenSubKey("Remote Recorder").GetValue("Home Directory").ToString() + @"log\CiscoAsaV6_4_1Recorder" + Id + ".log";
                rk.Close();
                return true;
            }
            catch (Exception er)
            {
                EventLog.WriteEntry("Security Manager CiscoAsaV6_4_1Recorder Recorder Read Registry", er.ToString(), EventLogEntryType.Error);
                return false;
            }
            finally
            {
        ...
        public CiscoAsaSyslogV6_4_1Recorder()
        {
        }
        ...
        public bool Initialize_Logger()
        {
            ...
            {
                EventLog.WriteEntry("Security Manager CiscoAsaV6_4_1Recorder Recorder", er.ToString(), EventLogEntryType.Error);
                return false;
            }
        }
