...
namespace CiscoAsa112SyslogRecorder
{
    public class CiscoAsa112SyslogRecorder : CustomBase
    {
        ...
        String CustomVar1, int CustomVar2, String Virtualhost, String dal, Int32 Zone)
        {
            ...
            zone = Zone;
            ParsArgs(CustomVar1);
            if(typeRegex == null) typeRegex = new Regex(@"%ASA[^:]*-([0-9]+):",RegexOptions.Compiled);
        }

        protected void ParsArgs(string args)
        {
            var regex = new Regex(@"\s*([\w]*)\s*=\s*(.*)\s*;", RegexOptions.Compiled);
            ...
            while (match.Success)
            {
                ...
                {
                    case "TypeRegex" :
                        typeRegex = new Regex(match.Groups[2].Value,RegexOptions.Compiled);
                        break;
                }
        ...
        void slog_SyslogEvent(LogMgrEventArgs args)
        {
            ...
            try
            {
                ...
                //10.3.0.1:514 : local7.info :Jun 20 11:00:44 EEDT: %ASA-session-6-302016: Teardown UDP connection 20758 for interface2:10.3.40.144/64629 to interface1:DC2/53 duration 0:02:15 bytes 541
                ...
                var typeMatch = typeRegex.Match(args.Message);
