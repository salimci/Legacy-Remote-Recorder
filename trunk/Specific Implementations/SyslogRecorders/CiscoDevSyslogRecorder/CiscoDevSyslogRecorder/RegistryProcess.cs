    ...
    class RegistryProcess
    {
        ...
        public bool get_regisryforAgent()
        {
            ...
            try
            {
                ...
                err_log = rk.OpenSubKey("Agent").GetValue("Home Directory").ToString() + @"log\CiscoDevSyslogRecorder.log";
                syslog_port = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("CiscoDevSyslogRecorder").GetValue("Syslog Port"));
                trc_level = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("CiscoDevSyslogRecorder").GetValue("Trace Level"));
        ...
        public bool get_registryforRemoteRecorder(int Id, string location, int traceLevel) 
        {
            ...
            {
                rk = Registry.LocalMachine.OpenSubKey("SOFTWARE").OpenSubKey("Natek").OpenSubKey("Security Manager");
                err_log = rk.OpenSubKey("Remote Recorder").GetValue("Home Directory").ToString() + @"log\CiscoDevSyslogRecorder" + Id + ".log";
                rk.Close();
            }
            catch (Exception)
            {
