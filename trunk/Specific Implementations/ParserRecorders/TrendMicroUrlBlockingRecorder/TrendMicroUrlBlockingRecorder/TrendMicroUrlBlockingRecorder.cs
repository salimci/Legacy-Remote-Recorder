...
namespace Parser
{
    public class TrendMicroUrlBlockingRecorder : Parser
    {

        data args = null;

        public TrendMicroUrlBlockingRecorder()
            : base()
        {
            LogName = "TrendMicroUrlBlockingRecorder";
            usingKeywords = false;
            lineLimit = 50;
        }
        ...
        public override bool ParseSpecific(String line, bool dontSend)
        {
            ...
            if (!dontSend)
            {
                ...
                try
                {
                    ...
                    else if (arr[0] == "Scan_type")
                    {
                        args.Scan_type = value;
                    }
                    else if (arr[0] == "Content category")
                    {
                        args.ContentCategory = value;
                    }
                    else if (arr[0] == "Filtering Action")
                    {
                        args.FilteringAction = value;
                    }
                    ...
                    if (SendRecord)
                    {
                        ...
                        r.CustomStr1 = args.Url;
                        r.CustomStr2 = args.Rule;
                        r.CustomStr3 = args.Scan_type;
                        r.CustomStr4 = args.ContentCategory;
                        r.CustomStr5 = args.Category;
                        r.CustomStr6 = args.Method;
                        r.CustomInt1 = Convert_To_Int32(args.Opp_id);
                        r.CustomInt2 = Convert_To_Int32(args.FilteringAction);
        ...
        protected override void ParseFileNameLocal()
        {
            if (Dir.EndsWith("/") || Dir.EndsWith("\\"))
            {
                ...
                foreach (String file in Directory.GetFiles(Dir))
                {
                    ...
                    if (filename.StartsWith("url_blocking") == true )//Name changed
                    {
        ...
        protected override void ParseFileNameRemote()
        {
            ...
            try
            {
                ...
                if (Dir.EndsWith("/") || Dir.EndsWith("\\"))
                {
                    ...
                    String command = "ls -lt " + Dir + " | grep url_blocking";
                    ...
                    {
                        String[] arr = line.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                        if (arr[arr.Length - 1].StartsWith("url_blocking") == true && arr[arr.Length - 1].Split(new char[] { '.' }, StringSplitOptions.RemoveEmptyEntries).Length == 6)//Name changed
                        {
            ...
            try
            {
                for (int i = 0; i < arrFileNames.Count; i++)
                {
                    ...
                    if (arr.Length == 6)
                    {
                        ...
                        Int64 numberInt = Convert_To_Int64(numberStr);
                        
                        dFileNumberList[i] = Convert.ToUInt64(numberInt);
