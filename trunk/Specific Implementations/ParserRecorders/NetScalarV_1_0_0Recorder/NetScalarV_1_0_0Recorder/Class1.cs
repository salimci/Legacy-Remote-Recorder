...
namespace Parser
{
    ...
    public class NetScalarV_1_0_0Recorder : Parser//recorderName***
    {
        ...
        public NetScalarV_1_0_0Recorder()
            : base()
        {
            LogName = "NetScalarV_1_0_0Recorder";
        ...
        public NetScalarV_1_0_0Recorder(String fileName)//recorderName***
            : base(fileName)
        {
        ...
        public override bool ParseSpecific(String line, bool dontSend)
        {
            ...
            try
            {
                ...
                {
                    DateTime dt;
                    string myDateTimeString = lineArr[0] + " " + lineArr[1];
                ...
                {
                    r.CustomStr5 = lineArr[17];
                    Log.Log(LogType.FILE, LogLevel.DEBUG, "CustomStr5: " + r.CustomStr5);
                }
                if (lineArr.Length > 15)
                {
                    r.CustomStr8 = lineArr[14] + " " + lineArr[15];
                    Log.Log(LogType.FILE, LogLevel.DEBUG, "CustomStr8: " + r.CustomStr8);
                }
                if (lineArr.Length > 16)
                {
                    r.CustomStr9 = lineArr[16];
                    Log.Log(LogType.FILE, LogLevel.DEBUG, "CustomStr9: " + r.CustomStr9);
                }

                r.CustomStr1 = FileName;

                if (lineArr.Length > 6)
                {
                    r.CustomInt1 = IntegerParsingOperation(lineArr[6], 6, "CustomInt1", lineArr.Length);
                }

                if (lineArr.Length > 10)
                {
                    r.CustomInt2 = IntegerParsingOperation(lineArr[10], 10, "CustomInt2", lineArr.Length);
                }

                if (lineArr.Length > 11)
                {
                    r.CustomInt3 = IntegerParsingOperation(lineArr[11], 11, "CustomInt3", lineArr.Length);
                }

                if (lineArr.Length > 12)
                {
                    r.CustomInt4 = IntegerParsingOperation(lineArr[12], 12, "CustomInt4", lineArr.Length);
                }
        ...
        public int IntegerParsingOperation(string value, int index, string column, int lengthValue)
        {
            ...
            if (lengthValue > index)
            {
                try
                {
                    ...
                    Log.Log(LogType.FILE, LogLevel.DEBUG, " ParseSpecific() -- '" + column + "' : " + value);
                }
                catch (Exception exception)
                {
                    Log.Log(LogType.FILE, LogLevel.DEBUG, " ParseSpecific() -- '" + column + "' Error : " + exception.Message);
                }
            }
            return return_value;
