...
namespace Parser
{
    ...
    public class WamppServerErrorV2_2_0Recorder : Parser
    {
        ...
        public WamppServerErrorV2_2_0Recorder()
            : base()
        {
            LogName = "WamppServerErrorV2_2_0Recorder";
        ...
        public WamppServerErrorV2_2_0Recorder(String fileName)//recorderName***
            : base(fileName)
        {
        ...
        public override bool ParseSpecific(String line, bool dontSend)
        {
            ...
            try
            {
                ...
                if (!line.StartsWith("Invalid"))
                {
                    try
                    {
                        subLine = Between(line, "[", "]");
                        string findChar = "[";
                        int Found = (line.Length - line.Replace(findChar, "").Length) / findChar.Length;

                        if (Found == 2)
                        {
                            Log.Log(LogType.FILE, LogLevel.DEBUG, "Found: " + Found.ToString());
                            string[] lineArr = subLine.Split(']');
                            string[] dateArr = lineArr[0].Split(' ');
                            DateTime dt;
                            string myDateTimeString = dateArr[1] + dateArr[2] + "," + dateArr[4] + "," + dateArr[3];
                            dt = Convert.ToDateTime(myDateTimeString);
                            r.EventType = lineArr[1].Replace('[', ' ').Replace(']', ' ').Trim();
                        ...
                        else if (Found == 3)
                        {
                            ...
                            r.EventType = lineArr[1].Replace('[', ' ').Replace(']', ' ').Trim();
                            Log.Log(LogType.FILE, LogLevel.DEBUG, "EventType: " + r.EventType);
                            r.CustomStr3 = lineArr[2].Split(' ')[2];
                    ...
                    r.Description = After(line, "]");
                }
                else
                {
                    Log.Log(LogType.FILE, LogLevel.INFORM, "Log format is unrecognized.");
                }
