...
namespace Parser
{
    public class WebwasherFoundVirusRecorder : Parser
    {

        public WebwasherFoundVirusRecorder()
            : base()
        {
            LogName = "WebwasherFoundVirusRecorder";
            usingKeywords = false;
            lineLimit = 50;
        }
        ...
        protected override void ParseFileNameRemote()
        {
            ...
            try
            {
                ...
                if (Dir.EndsWith("/") || Dir.EndsWith("\\"))
                {
                    ...
                    String command = "ls -lt " + Dir + " | grep foundviruses";
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        if (arr[arr.Length - 1].StartsWith("foundviruses") == true && arr[arr.Length - 1].Split(new char[] { '.' }, StringSplitOptions.RemoveEmptyEntries).Length == 3)//Name changed
                        {
        ...
        private string[] SortFiles(ArrayList arrFileNames)
        {
            ...
            try
            {
                for (int i = 0; i < arrFileNames.Count; i++)
                {
                    ...
                    if (parts.Length == 3)
                    {
                        //foundviruses1010211545.merged
                        dFileNumberList[i] = Convert.ToUInt64(parts[0].Remove(0, 12));
