...
namespace OracleEnebisAuditV_1_0_0Recorder
{
    ...
    public class OracleEnebisAuditV_1_0_0Recorder : CustomBase
    {
        public Fields RecordFields;
        ...
        public OracleEnebisAuditV_1_0_0Recorder()
        {
            InitializeComponent();
        ...
        public override void Init()
        {
                ...
                if (usingRegistry)
                {
                    if (!reg_flag)
                    {
                        if (!Read_Registry())
                        {
                            ...
                            {
                                L.Log(LogType.FILE, LogLevel.ERROR, "Error on Intialize Logger on OracleAudit Recorder functions may not be running");
                                return;
                            }
                        reg_flag = true;
                ...
                else
                {
                    if (!reg_flag)
                    {
                        if (!Get_logDir())
                        {
                            ...
                            if (!Initialize_Logger())
                            {
                                L.Log(LogType.FILE, LogLevel.ERROR, "Error on Intialize Logger on OracleAuditRecorder  functions may not be running");
                                return;
                            }
                        L.Log(LogType.FILE, LogLevel.INFORM, "Start creating OracleAuditRecorder DAL");
            ...
            {
                EventLog.WriteEntry("Security Manager OracleAuditRecorder Recorder Init", er.ToString(), EventLogEntryType.Error);
            }
        }
        ...
        public bool Get_logDir()
        {
            ...
            try
            {
                ...
                err_log = rk.OpenSubKey("Remote Recorder").GetValue("Home Directory").ToString() + @"log\OracleAuditRecorder" + Id + ".log";
                rk.Close();
                return true;
            }
            catch (Exception er)
            {
                EventLog.WriteEntry("Security Manager OracleAuditRecorder Recorder Read Registry", er.ToString(), EventLogEntryType.Error);
                return false;
            }
            finally
            {
        ...
        public bool Read_Registry()
        {
            ...
            {
                rk = Registry.LocalMachine.OpenSubKey("SOFTWARE").OpenSubKey("Natek").OpenSubKey("Security Manager");
                L.Log(LogType.FILE, LogLevel.INFORM, " Read_Registry -->> rk : " + rk);
                log_size = Convert.ToUInt32(rk.OpenSubKey("Recorder").OpenSubKey("OracleAuditRecorder").GetValue("Log Size"));
                L.Log(LogType.FILE, LogLevel.INFORM, " Read_Registry -->> log_size : " + log_size);
                logging_interval = Convert.ToUInt32(rk.OpenSubKey("Recorder").OpenSubKey("OracleAuditRecorder").GetValue("Logging Interval"));
                L.Log(LogType.FILE, LogLevel.INFORM, " Read_Registry -->> logging_interval : " + logging_interval);

                trc_level = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("OracleAuditRecorder").GetValue("Trace Level"));
                L.Log(LogType.FILE, LogLevel.INFORM, " Read_Registry -->> trc_level : " + trc_level);
                err_log = rk.OpenSubKey("Agent").GetValue("Home Directory").ToString() + @"log\OracleAuditRecorder.log";
                L.Log(LogType.FILE, LogLevel.INFORM, " Read_Registry -->> err_log : " + err_log);
                db_name = rk.OpenSubKey("Recorder").OpenSubKey("OracleAuditRecorder").GetValue("DBName").ToString();
                L.Log(LogType.FILE, LogLevel.INFORM, " Read_Registry -->> db_name : " + db_name);
                this.timer1.Interval = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("OracleAuditRecorder").GetValue("Interval"));
                L.Log(LogType.FILE, LogLevel.INFORM, " Read_Registry -->> db_name : " + db_name);
                //
                wsdb_name = rk.OpenSubKey("Recorder").OpenSubKey("OracleAuditRecorder").GetValue("WSDBName").ToString();
                L.Log(LogType.FILE, LogLevel.INFORM, " Read_Registry -->> wsdb_name : " + wsdb_name);
                max_record_send = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("OracleAuditRecorder").GetValue("MaxRecordSend"));
                L.Log(LogType.FILE, LogLevel.INFORM, " Read_Registry -->> max_record_send : " + max_record_send);
                last_recordnum = Convert.ToInt64(rk.OpenSubKey("Recorder").OpenSubKey("OracleAuditRecorder").GetValue("LastRecordNum"));
                L.Log(LogType.FILE, LogLevel.INFORM, " Read_Registry -->> last_recordnum : " + last_recordnum);



                //RecordFields.oracleUserName = rk.OpenSubKey("Recorder").OpenSubKey("OracleAuditRecorder").GetValue("User").ToString();
                //RecordFields.oracleHost = rk.OpenSubKey("Recorder").OpenSubKey("OracleAuditRecorder").GetValue("Host").ToString();
                //RecordFields.oraclePassword = rk.OpenSubKey("Recorder").OpenSubKey("OracleAuditRecorder").GetValue("Password").ToString();
                //RecordFields.oracleDb = rk.OpenSubKey("Recorder").OpenSubKey("OracleAuditRecorder").GetValue("DB").ToString();
        ...
        private void timer1_Tick(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            L.Log(LogType.FILE, LogLevel.DEBUG, " timer1_Tick -->> virtualhost: ");
            L.Log(LogType.FILE, LogLevel.DEBUG, " timer1_Tick -->> last_recordnum: " + last_recordnum);
            ...
            try
            {
                ...
                while (readReader.Read())
                {
                    ...
                    rec.LogName = "OracleEnebisAuditV_1_0_0Recorder";
                    string timestamp = "";
                    ...
                    string str9 = Convert_To_String(readReader["SQLBIND"]);
                    ...
                    last_recordnum = Convert_To_Int64(readReader["REC_ID"].ToString());
        ...
        private long Convert_To_Int64(string value)
        {
            ...
            {
                //L.Log(LogType.FILE, LogLevel.INFORM, " Convert_To_Int64 -->> Result: " + result);
                return result;
            }

            else
            {
                //L.Log(LogType.FILE, LogLevel.INFORM, " Convert_To_Int64 -->> value can not convert to int64. Result returned 0. " );
                return 0;
            }
        }
        ...
        public bool Set_Registry(long status)
        {
            ...
            try
            {
                rk = Registry.LocalMachine.CreateSubKey("SOFTWARE").CreateSubKey("Natek").CreateSubKey("Security Manager").CreateSubKey("Recorder").CreateSubKey("OracleAuditRecorder");
