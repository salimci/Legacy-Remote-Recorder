...
{
    public class NatekMdmStagingRecorder : CustomBase
    {
        ...
        public NatekMdmStagingRecorder()
        {
            _sync = new object();
            _status = 0;
        ...
        static Regex REG_SQL = new Regex("^\\[[0-9]*m(.*)$", RegexOptions.Compiled);
        private bool ProcessRecord(string line, List<string> record)
        {
            try
            {
                GetRecord(line, record);
                CustomServiceBase sender = null;
                if (record.Count > 1)
                {
                    var sql = GetField(record, 4);
                    var m = REG_SQL.Match(sql);
                    if (m.Success)
                        sql = m.Groups[1].Value;
                    var rec = new Rec()
                    {
                        LogName = "NatekMdmStagingRecorder",
                    ...
                    try
                    {
                        sender = GetInstanceService("Security Manager Remote Recorder");
                        sender.SetData(_dal, _virtualhost, rec);
                    }
                    catch (Exception ie)
                    {
                        _logger.Log(LogType.FILE, LogLevel.ERROR,
                                    "Error ProcessRecord Inner: [" + line + "] => " + ie.Message);
                        return false;
                    }
                }
                else
                {
                    ...
                    {
                        sender = GetInstanceService("Security Manager Remote Recorder");
                        sender.SetReg(_identity, (_lastPosition + 1).ToString(), line, _lastFile, string.Empty);
                    }
                    catch (Exception re)
                    {
                        _logger.Log(LogType.FILE, LogLevel.ERROR,
                                    "Error ProcessRecord Garbage Line: [" + line + "] => " + re.Message);
                    }
                }
                ++_lastPosition;
        ...
        private List<string> GetRecord(string line, List<string> record)
        {
            ...
            while (++curr < line.Length)
            {
                if (line[curr] == 27)
                {
                    record.Add(line.Substring(back, curr - back));
                    back = curr + 1;
                }
            }
            record.Add(line.Substring(back, curr - back));
        ...
        public string ReadLogFilename()
        {
            try
            {
                ...
                {
                    return Path.Combine(Path.Combine(reg.GetValue("Home Directory").ToString(), "log"), "NatekMdmStagingRecorder" + _identity + ".log");
                }
            }
            catch (Exception e)
            {
                EventLog.WriteEntry("Security Manager NatekMdmStagingRecorder Read Registry", e.ToString(), EventLogEntryType.Error);
            }
            return null;
        }
