...
//namespace KombsOracleDBRecorder
//{
...
//    public class KombsOracleDBRecorder : CustomBase
//    {
...
//                err_log = rk.OpenSubKey("Remote Recorder").GetValue("Home Directory").ToString() + @"log\KombsOracleDBRecorder" + Id + ".log";
//                rk.Close();
//                return true;
//            }
//            catch (Exception er)
//            {
//                EventLog.WriteEntry("Security Manager KombsOracleDBRecorder Read Registry", er.ToString(), EventLogEntryType.Error);
//                return false;
//            }
//            finally
//            {
...
//            //{
//            //    rk = Registry.LocalMachine.OpenSubKey("SOFTWARE").OpenSubKey("Natek").OpenSubKey("Security Manager");
//            //    log_size = Convert.ToUInt32(rk.OpenSubKey("Recorder").OpenSubKey("KombsOracleDBRecorder").GetValue("Log Size"));
//            //    logging_interval = Convert.ToUInt32(rk.OpenSubKey("Recorder").OpenSubKey("KombsOracleDBRecorder").GetValue("Logging Interval"));
//            //    trc_level = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("KombsOracleDBRecorder").GetValue("Trace Level"));
//            //    err_log = rk.OpenSubKey("Agent").GetValue("Home Directory").ToString() + @"log\KombsOracleDBRecorder.log";
//            //    db_name = rk.OpenSubKey("Recorder").OpenSubKey("KombsOracleDBRecorder").GetValue("DBName").ToString();
//            //    this.timer1.Interval = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("KombsOracleDBRecorder").GetValue("Interval"));
//            //    wsdb_name = rk.OpenSubKey("Recorder").OpenSubKey("KombsOracleDBRecorder").GetValue("WSDBName").ToString();
//            //    max_record_send = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("KombsOracleDBRecorder").GetValue("MaxRecordSend"));
//            //    last_recordnum = Convert.ToInt64(rk.OpenSubKey("Recorder").OpenSubKey("KombsOracleDBRecorder").GetValue("LastRecordNum"));
//            //    rk.Close();
//            //    return true;
//            //}
//            //catch (Exception er)
...
//                readQuery = "Select MIN(ID) from KOMBSLOG";
...
//                readReader.Read();
//                RecordFields.datacount = Convert_To_Int64(readReader[0].ToString());
//                readReader.Close();

//                L.Log(LogType.FILE, LogLevel.INFORM, " timer1_Tick -->> Total data count in sys.enbs_audit table : " + RecordFields.datacount.ToString(CultureInfo.InvariantCulture));
//                //L.Log(LogType.FILE, LogLevel.INFORM, " timer1_Tick -->> Last data number read from sys.enbs_audit table is : " + last_recordnum.ToString(CultureInfo.InvariantCulture));

//                if (RecordFields.datacount == 1)
//                {
//                    readQuery = string.Concat(new object[] { "select * from (select TARIH, SAATI, SICILI, BELGE_NO, BILGISAYARADI, SQL, ISLEM, IP from KOMBSLOG) where DATANUM >=0 and DATANUM <1000" });
//                }
//                else
//                {
//                    readQuery = string.Concat(new object[] { "select * from KOMBSLOG Where Id > " + RecordFields.datacount });
//                    RecordFields.datacount++;
//                }
...
//                //    readQuery = string.Concat(new object[] { "select * from KOMBSLOG" });
//                //}
//                //else
//                //{
//                //    L.Log(LogType.FILE, LogLevel.DEBUG, " timer1_Tick -->> is Last Pozision != 0 ");
//                //    readQuery = string.Concat(new object[] { "select * from KOMBSLOG WHERE ID > " + datacount });
//                //    last_recordnum++;
//                //}
//                L.Log(LogType.FILE, LogLevel.INFORM, " Onur last_recordnum: " + last_recordnum);
...
//                {
//                    rec.LogName = "KombsOracleDBRecorder";
...
//                    try
//                    {
//                        DateTime dt1 = Convert.ToDateTime(readReader["TARIH"].ToString());
//                        string dt2 = dt1.ToString("yyyy-MM-dd");
//                        date = dt2;
//                        time = Convert_To_String(readReader["SAATI"]);
...
//                    try
//                    {
//                        rec.CustomInt1 = Convert_To_Int32(readReader["SICILI"].ToString());
//                        L.Log(LogType.FILE, LogLevel.INFORM, " SICILI -->> : " + readReader["SICILI"]);
//                    }
//                    catch (Exception ex)
//                    {
//                        rec.CustomInt1 = 0;
...
//                    try
//                    {
//                        rec.CustomInt2 = Convert_To_Int32(readReader["BELGE_NO"].ToString());
//                        L.Log(LogType.FILE, LogLevel.INFORM, " BELGE_NO -->> : " + readReader["BELGE_NO"]);
//                    }
//                    catch (Exception ex)
//                    {
//                        rec.CustomInt2 = 0;
...
//                    rec.CustomStr10 = Convert_To_String(readReader["SQL"].ToString());
//                    rec.CustomStr2 = Convert_To_String(readReader["ISLEM"].ToString());
//                    rec.CustomStr3 = Convert_To_String(readReader["IP"].ToString());
//                    rec.CustomStr4 = Convert_To_String(readReader["BILGISAYARADI"].ToString());
...
//                        L.Log(LogType.FILE, LogLevel.INFORM, " timer1_Tick -->> Finish Sending Data");
//                    }


//                    L.Log(LogType.FILE, LogLevel.INFORM, " timer1_Tick -->> Start sending Data. Last Record Number : " + RecordFields.datacount);

//                    if (usingRegistry)
//                        Set_Registry(RecordFields.datacount);
//                    else
//                    {
//                        CustomServiceBase s = base.GetInstanceService("Security Manager Remote Recorder");
//                        s.SetReg(Id, RecordFields.datacount.ToString(CultureInfo.InvariantCulture), "", "", "", LastRecordDate);
//                    }
//                }
    ...
    public class KombsOracleDBRecorder : CustomBase
    {
        ...
        private void timer1_Tick(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            try
            {
                ...
                readQuery = "select COUNT(*) from KOMBSLOG";
                L.Log(LogType.FILE, LogLevel.DEBUG, " timer1_Tick -->> readQuery : " + readQuery);
                ...
                if (last_recordnum == 0)
                {
                    //(select TARIH, SAATI, SICILI, BELGE_NO, BILGISAYARADI, SQL, ISLEM, IP from KOMBSLOG)
                    L.Log(LogType.FILE, LogLevel.DEBUG, " timer1_Tick -->> is Last Pozision == 0 ");
                    ...
                    readQuery = string.Concat(new object[] { "select * from (select TARIH, SAATI, SICILI, BELGE_NO, BILGISAYARADI, SQL, ISLEM, IP, ROWNUM as DATANUM from KOMBSLOG"+
                                                                ") where DATANUM >=", last_recordnum, " and DATANUM <", last_recordnum + Convert_To_Int64(max_record_send.ToString(CultureInfo.InvariantCulture)), " " });

                    L.Log(LogType.FILE, LogLevel.INFORM, "Onur -->> ReadQuerry 1");
                ...
                else
                {
                    ...
                    L.Log(LogType.FILE, LogLevel.INFORM, "Onur -->> ReadQuerry 2");
                }
                ...
                while (readReader.Read())
                {
                    ...
                    // log satırı koy recordnumLara bak,
                    L.Log(LogType.FILE, LogLevel.INFORM, "Onur -->> last_recordnum : " + last_recordnum);
                    ...
                    {
                        rec.CustomInt1 = Convert_To_Int32(readReader["SICILI"].ToString());
                        L.Log(LogType.FILE, LogLevel.DEBUG, " SICILI -->> : " + readReader["SICILI"]);
                    }
                    catch (Exception ex)
                    {
                        rec.CustomInt1 = 0;
                    ...
                    {
                        rec.CustomInt2 = Convert_To_Int32(readReader["BELGE_NO"].ToString());
                        L.Log(LogType.FILE, LogLevel.DEBUG, " BELGE_NO -->> : " + readReader["BELGE_NO"]);
                    }
                    catch (Exception ex)
                    {
                        rec.CustomInt2 = 0;
                    ...
                    if (usingRegistry)
                    {
                        CustomServiceBase s = base.GetInstanceService("Security Manager Sender");
                        s.SetData(rec);
                        L.Log(LogType.FILE, LogLevel.INFORM, " Security Manager Sender Using Registry -->> : ");
                    }
                    else
                    {
                        CustomServiceBase s = base.GetInstanceService("Security Manager Remote Recorder");
                        ...
                        L.Log(LogType.FILE, LogLevel.INFORM, " Security Manager Remote Recorder DAL : -->> : " + Dal);
                        L.Log(LogType.FILE, LogLevel.INFORM, " Security Manager Remote Recorder  virtualhost : -->> : " + virtualhost);
                    }

                    L.Log(LogType.FILE, LogLevel.INFORM, " timer1_Tick -->> Finish Sending Data");

                    LastRecordDate = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");

                    if (usingRegistry)
                    {
                        Set_Registry(last_recordnum);
                        L.Log(LogType.FILE, LogLevel.INFORM, "Onur -->> Using Registry. " );
                    }
                        
                    else
                    {
                        CustomServiceBase s = base.GetInstanceService("Security Manager Remote Recorder");
                        L.Log(LogType.FILE, LogLevel.INFORM, "Onur -->> 1");
                        s.SetReg(Id, last_recordnum.ToString(), "", "", "", LastRecordDate);
                        L.Log(LogType.FILE, LogLevel.INFORM, "Onur -->> 2");
                        L.Log(LogType.FILE, LogLevel.INFORM, "Onur -->> Not Using Registry, Id = " + Id + " last_recordnum = " + last_recordnum + " LastRecordDate = " + LastRecordDate);
                    }
                }
