...
namespace Parser
{
    public class UbuntuauthRecorder : Parser
    {
        public UbuntuauthRecorder()
            : base()
        {
            LogName = "UbuntuauthRecorder";
            usingKeywords = false;
        }
        ...
        public UbuntuauthRecorder(String fileName)
            : base(fileName)
        {
        }
        ...
        protected override void ParseFileNameRemote()
        {
            try
            {
                Log.Log(LogType.FILE, LogLevel.INFORM, " UbuntuauthRecorder In ParseFileNameRemote() -->> Enter The Function");
                ...
                if (Dir.EndsWith("/") || Dir.EndsWith("\\"))
                {
                    ...
                    Log.Log(LogType.FILE, LogLevel.DEBUG, " UbuntuauthRecorder In ParseFileNameRemote() -->> SSH command : " + command);
                    ...
                    {
                        Log.Log(LogType.FILE, LogLevel.DEBUG, " UbuntuauthRecorder In ParseFileNameRemote() -->> LastFile Is = " + lastFile);
                        ...
                        if (bLastFileExist)
                        {
                            ...
                            if (readMethod == "nread")
                            {
                                ...
                                Log.Log(LogType.FILE, LogLevel.DEBUG, " UbuntuauthRecorder In ParseFileNameRemote() -->> commandRead For nread Is : " + commandRead);
                            }
                            else
                            {
                                commandRead = readMethod + " -n " + Position + "," + lineLimit + "p " + lastFile;
                                Log.Log(LogType.FILE, LogLevel.DEBUG, " UbuntuauthRecorder In ParseFileNameRemote() -->> commandRead For sed Is : " + commandRead);
                            }
                            ...
                            while ((lineTest = srTest.ReadLine()) != null)
                            {
                                if (lineTest.StartsWith("~?`Position"))
                                {
                                    ...
                                    {
                                        Log.Log(LogType.FILE, LogLevel.ERROR, " UbuntuauthRecorder In ParseFileNameRemote() In Try Catch -->> " + ex.Message);
                                    }
                                }
                            }
                            ...
                            if (posTest > Position)
                            {
                                Log.Log(LogType.FILE, LogLevel.INFORM,
                                    " UbuntuauthRecorder In ParseFileNameRemote() -->> posTest > Position So Continiou With Same File ");

                                FileName = lastFile;

                                Log.Log(LogType.FILE, LogLevel.INFORM,
                                    " UbuntuauthRecorder In ParseFileNameRemote() -->> LastFile Is " + lastFile);
                            }
                            else
                            {

                                Log.Log(LogType.FILE, LogLevel.INFORM,
                                   " UbuntuauthRecorder In ParseFileNameRemote() -->> Finished Reading The File");
                                ...
                                for (int i = 0; i < dFileNameList.Length; i++)
                                {
                                    if (Dir + dFileNameList[i].ToString() == lastFile)
                                    {
                                        if (i + 1 == dFileNameList.Length)
                                        {
                                            FileName = lastFile;
                                            Log.Log(LogType.FILE, LogLevel.INFORM,
                                                " UbuntuauthRecorder In ParseFileNameRemote() -->> There Is No New File And Continiou With Same File And Waiting For a New Record " + FileName);
                                            break;
                                        }
                                        else
                                        {
                                            ...
                                            lastFile = FileName;
                                            Log.Log(LogType.FILE, LogLevel.INFORM,
                                                " UbuntuauthRecorder In ParseFileNameRemote() -->> Finished Reading The File And Continiou With New File " + FileName);
                                            break;
                    ...
                    else
                    {
                        ...
                        if (dFileNameList.Length > 0)
                        {
                            ...
                            Position = 0;
                            Log.Log(LogType.FILE, LogLevel.INFORM,
                                "  UbuntuauthRecorder In ParseFileNameRemote() -->>  Last File Is Null And Setted The File To " + FileName);
            ...
            {
                Log.Log(LogType.FILE, LogLevel.ERROR, "  UbuntuauthRecorder In ParseFileNameRemote() In Catch -->>" + exp.Message);
                Log.Log(LogType.FILE, LogLevel.ERROR, "  UbuntuauthRecorder In ParseFileNameRemote() In Catch -->>" + exp.StackTrace);
                return;
            }

            Log.Log(LogType.FILE, LogLevel.INFORM, " UbuntuauthRecorder In ParseFileNameRemote() -->>  Exit The Function");
        }
        ...
        public override void GetFiles()
        {
            ...
            catch (Exception ex)
            {
                if (reg == null)
                {
                    Log.Log(LogType.EVENTLOG, LogLevel.ERROR, "  UbuntuauthRecorder In GetFiles() Exception Message -->> " + ex.Message);
                    Log.Log(LogType.EVENTLOG, LogLevel.ERROR, "  UbuntuauthRecorder In GetFiles() Exception StackTrace -->> " + ex.StackTrace);
                }
                else
                {
                    Log.Log(LogType.FILE, LogLevel.ERROR, "  UbuntuauthRecorder In GetFiles() Exception Message -->> " + ex.Message);
                    Log.Log(LogType.FILE, LogLevel.ERROR, "  UbuntuauthRecorder In GetFiles() Exception StackTrace -->> " + ex.StackTrace);
                }
            }
        }
