...
namespace Parser
{
    public class WebwasherAuditRecorder : Parser
    {

        public WebwasherAuditRecorder()
            : base()
        {
            LogName = "WebwasherAuditRecorder";
            usingKeywords = false;
            lineLimit = 50;
        }
        ...
        public override bool ParseSpecific(String line, bool dontSend)
        {
            ...
            if (!dontSend)
            {
                ...
                //[20/Oct/2010:08:08:10 +0000] [HTTPS] Admin 7sp0L1NntfXkNw3 192.168.20.166 "setting changed" "global" LogFiles.EnableAccess "0" "2" /conf?navTo=ActivateLogFiles
                    Rec r = new Rec();
                    r.Description = line;
                    r.LogName = LogName;
                ...
                try
                {
                    if (arr.Length >= 9)
                    {
                        string[] dateParts = arr[0].Split(new char[] { ' ', ':' }, StringSplitOptions.RemoveEmptyEntries);
                        ...
                        r.CustomStr2 = arr[2].TrimStart('[').TrimEnd(']');
                        r.UserName = arr[3] + " " + arr[4];
                        r.SourceName = arr[5];

                        //"setting changed" "global" UserMapping-User-Direct-1.Method "-" "2" /conf?navTo=WebMappingMethods
                        String[] parts = line.Split(new char[] { '"' }, StringSplitOptions.RemoveEmptyEntries);
                        if (parts.Length == 6)
                        {
                            r.EventType = parts[1];
                            r.EventCategory = parts[2];
                            r.CustomStr1 = parts[3];
                            r.CustomStr10 = "";
        ...
        protected override void ParseFileNameRemote()
        {
            ...
            try
            {
                ...
                if (Dir.EndsWith("/") || Dir.EndsWith("\\"))
                {
                    ...
                    String command = "ls -lt " + Dir + " | grep audit";
                    ...
                    {
                        String[] arr = line.Split(new char[] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
                        if (arr[arr.Length - 1].StartsWith("audit") == true && arr[arr.Length - 1].Split(new char[] { '.' }, StringSplitOptions.RemoveEmptyEntries).Length == 3)//Name changed
                        {
        ...
        private string[] SortFiles(ArrayList arrFileNames)
        {
            ...
            try
            {
                for (int i = 0; i < arrFileNames.Count; i++)
                {
                    ...
                    if (parts.Length == 3)
                    {   //audit1012300925.merged
                        dFileNumberList[i] = Convert.ToUInt64(parts[0].Remove(0, 5));
