...
namespace Parser
{
    ...
    public class NPS2008V_1_0_0Recorder : Parser//recorderName***
    {
        ...
        public NPS2008V_1_0_0Recorder()
            : base()
        {
            LogName = "NPS2008V_1_0_0Recorder";
        ...
        public NPS2008V_1_0_0Recorder(String fileName)//recorderName***
            : base(fileName)
        {
        ...
        public override bool ParseSpecific(String line, bool dontSend)
        {
            ...
            for (int i = 0; i < lineArr.Length; i++)
            {
                ...
                {
                    Log.Log(LogType.FILE, LogLevel.DEBUG, "LineArr : " + i + ". " + lineArr[i]);
                }
            }
            ...
            try
            {
                ...
                try
                {
                    string myDateTimeString = lineArr[2].Split('/')[2] + "-" + lineArr[2].Split('/')[0] + "-" + lineArr[2].Split('/')[1] + " " + lineArr[3];
                    DateTime dateTime = DateTime.ParseExact(myDateTimeString, dateFormat, CultureInfo.InvariantCulture);
                    r.Datetime = dateTime.ToString(dateFormat);
                ...
                try
                {
                    if (!string.IsNullOrEmpty(lineArr[60]))
                    {
                        r.SourceName = lineArr[60].Replace('"', ' ').Trim();
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "SourceName: " + r.SourceName);
                    }

                    if (!string.IsNullOrEmpty(lineArr[1]))
                    {
                        r.EventCategory = lineArr[1].Replace('"', ' ').Trim();
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "EventCategory: " + r.EventCategory);
                    }

                    if (!string.IsNullOrEmpty(lineArr[20]))
                    {
                        r.EventType = lineArr[20].Split(' ')[0];
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "EventType: " + r.EventType);
                    }

                    if (!string.IsNullOrEmpty(lineArr[5]))
                    {
                        r.UserName = lineArr[5].Replace('"', ' ').Trim();
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "UserName: " + r.UserName);
                    }
                    else
                    {
                        if (!string.IsNullOrEmpty(lineArr[6]))
                        {
                            r.UserName = lineArr[6].Split('\\')[1].Replace('"', ' ').Trim();
                            Log.Log(LogType.FILE, LogLevel.DEBUG, "UserName - Else: " + r.UserName);
                        }
                    }

                    if (!string.IsNullOrEmpty(lineArr[11]))
                    {
                        r.ComputerName = lineArr[11].Replace('"', ' ').Trim();
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "ComputerName: " + r.ComputerName);
                    }

                    if (!string.IsNullOrEmpty(lineArr[16]))
                    {
                        r.CustomStr1 = lineArr[16].Replace('"', ' ').Trim();
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "CustomStr1: " + r.CustomStr1);
                    }


                    if (!string.IsNullOrEmpty(lineArr[7]))
                    {
                        r.CustomStr2 = lineArr[7].Split(':')[1].Replace('"', ' ').Trim();
                    ...
                    else
                    {
                        if (!string.IsNullOrEmpty(lineArr[6]))
                        {
                            r.CustomStr5 = lineArr[6].Replace('"', ' ').Trim();
                            Log.Log(LogType.FILE, LogLevel.DEBUG, "CustomStr5 - Else: " + r.CustomStr5);
                        }
                    }
                    if (!string.IsNullOrEmpty(lineArr[15]))
                    {
                        r.CustomStr3 = lineArr[15].Replace('"', ' ').Trim();
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "CustomStr3: " + r.CustomStr3);
                    }

                    if (!string.IsNullOrEmpty(lineArr[0]))
                    {
                        r.CustomStr4 = lineArr[0].Replace('"', ' ').Trim();
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "CustomStr4: " + r.CustomStr4);
                    }

                    if (!string.IsNullOrEmpty(lineArr[8]))
                    {
                        r.CustomStr6 = lineArr[8].Replace('"', ' ').Trim();
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "CustomStr6: " + r.CustomStr6);
                    }
                    if (!string.IsNullOrEmpty(lineArr[20]))
                    {
                        r.CustomStr8 = lineArr[20].Split(' ')[1].Replace('"', ' ').Trim();
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "CustomStr8: " + r.CustomStr8);
                    }

                    if (!string.IsNullOrEmpty(lineArr[6]))
                    {
                        if (lineArr[6].Contains("/"))
                        {
                            string str9Line = lineArr[6];
                            string findChar = "/";
                            int Found = (str9Line.Length - str9Line.Replace(findChar, "").Length) / findChar.Length;
                            if (Found == 2)
                            {
                                r.CustomStr9 = lineArr[6].Split('/')[2].Replace('"', ' ').Trim();
                                Log.Log(LogType.FILE, LogLevel.DEBUG, "CustomStr9: " + r.CustomStr9);
                            }
                        }
                    }

                    if (!string.IsNullOrEmpty(lineArr[24]))
                    {
                        r.CustomStr10 = lineArr[24].Replace('"', ' ').Trim();
                    }
                ...
                {
                    Log.Log(LogType.FILE, LogLevel.ERROR, "String Parsing Error: " + exception.Message);
                }
