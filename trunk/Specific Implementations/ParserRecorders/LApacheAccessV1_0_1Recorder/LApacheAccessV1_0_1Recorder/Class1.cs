...
namespace Parser
{
    ...
    public class LApacheAccessV1_0_1Recorder : Parser
    {
        private string dateFormat = "yyyy-MM-dd HH:mm:ss";
        private String[] dFileNameList;
        private int port = 22;

        public LApacheAccessV1_0_1Recorder()
            : base()
        {
            LogName = "LApacheAccessV1_0_1Recorder";
            usingKeywords = false;
            lineLimit = 50;
        }

        public LApacheAccessV1_0_1Recorder(String fileName)
            : base(fileName)
        {
        ...
        public override bool ParseSpecific(string line, bool dontSend)
        {
            ...
            if (!dontSend)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        string dater = Between(line, "[", "]").Split('+')[0].Trim();
                        string[] lineArr1 = dater.Split(':');

                        string month = lineArr1[0].Split('/')[1];
                        string day = lineArr1[0].Split('/')[0];
                        string year = lineArr1[0].Split('/')[2];
                        string time = After(dater, ":", 1);
                        DateTime dt212;
                        string myDateTimeString = year + ", " + month + "," + day + "  ," + time;

                        dt212 = Convert.ToDateTime(myDateTimeString);
                        string date = dt212.ToString(dateFormat);
                        r.Datetime = date;
                    ...
                    {
                        Log.Log(LogType.FILE, LogLevel.ERROR, "Level_1" + exception.Message);
                    }
                    ...
                    {
                        Log.Log(LogType.FILE, LogLevel.ERROR, "Level_2" + exception.Message);
                    }
                    ...
                    {
                        Log.Log(LogType.FILE, LogLevel.ERROR, "Level_3" + exception.Message);
                    }
                    ...
                    {
                        Log.Log(LogType.FILE, LogLevel.ERROR, "Level_4" + exception.Message);
                    }
                    ...
                    {
                        Log.Log(LogType.FILE, LogLevel.ERROR, "Level_5" + exception.Message);
                    }
                    ...
                    try
                    {
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "Start sending Data, Position: " + Position);
                        SetRecordData(r);
        ...
        private bool Is_File_Finished(string file)
        {
            ...
            try
            {
                if (readMethod == "nread")
                {
                    ...
                    Log.Log(LogType.FILE, LogLevel.DEBUG, " Is_File_Finished() -->> Okunacak satyr sayysyna bakylyyor.");
                    ...
                    Log.Log(LogType.FILE, LogLevel.DEBUG, " Is_File_Finished() -->> Okunacak satyr sayysy bulundu. En az: " + lineCount);
                }
                else
                {
                    commandRead = "sed" + " -n " + Position + "," + (lineLimit + 2) + "p " + file;
                    Log.Log(LogType.FILE, LogLevel.DEBUG, " Is_File_Finished() -->> commandRead For nread Is : " + commandRead);
        ...
        private string[] SortFiles(ArrayList arrFileNames)
        {
            ...
            try
            {
                ...
                Log.Log(LogType.FILE, LogLevel.DEBUG, "SortFiles() -->> Syralanmy? dosya isimleri yazylyyor.");
                for (int i = 0; i < dFileNameList.Length; i++)
                {
