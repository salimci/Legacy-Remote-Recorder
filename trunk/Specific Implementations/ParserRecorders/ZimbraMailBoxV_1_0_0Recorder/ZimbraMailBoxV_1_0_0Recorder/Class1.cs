...
namespace Parser
{
    ...
    public class ZimbraMailBoxV_1_0_0Recorder : Parser
    {
        private string dateFormat = "yyyy/MM/dd HH:mm:ss";
        
        private Fields RecordFields;

        public ZimbraMailBoxV_1_0_0Recorder()
            : base()
        {
            LogName = "ZimbraMailBoxV_1_0_0Recorder";
            RecordFields = new Fields();
            usingKeywords = false;
            lineLimit = 50;
        }

        public ZimbraMailBoxV_1_0_0Recorder(String fileName)
            : base(fileName)
        {
        ...
        public override bool ParseSpecific(String line, bool dontSend)
        {
            ...
            if (!dontSend)
            {
                ...
                #region UserName
                string userName = Between(line, "[name=", "ip=").Trim();
...
//                rec.UserName = Between(line, "[name=", "ip=").Replace(';', ' ').Trim();
                ...
                string message = After(line, ";]");

                if (message.Contains("-"))
                {
                    rec.CustomStr8 = message.Split('-')[0];
                    rec.CustomStr9 = message.Split('-')[1];
                }
                else
                {
                    rec.CustomStr8 = message;
                }
