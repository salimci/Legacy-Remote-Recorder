//Name: OracleOnayTDHLogVW1 Recorder
...
namespace OracleOnayTDHLogVW1V_1_0_0Recorder
{
    public class OracleOnayTDHLogVW1V_1_0_0Recorder : CustomBase
    {
        ...
        public OracleOnayTDHLogVW1V_1_0_0Recorder()
        {
            InitializeComponent();
        ...
        public override void Init()
        {
            ...
            try
            {
                if (usingRegistry)
                {
                    if (!reg_flag)
                    {
                        if (!Read_Registry())
                        {
                            ...
                            {
                                L.Log(LogType.FILE, LogLevel.ERROR, "Error on Intialize Logger on OracleOnayLog Recorder functions may not be running");
                                return;
                            }
                        reg_flag = true;
                ...
                else
                {
                    if (!reg_flag)
                    {
                        ...
                        L.Log(LogType.FILE, LogLevel.INFORM, "Start creating OracleOnayTDHLogVW1 DAL");
            ...
            {
                EventLog.WriteEntry("Security Manager OracleOnayTDHLogVW1 Recorder Init", er.ToString(), EventLogEntryType.Error);
                L.Log(LogType.FILE, LogLevel.ERROR, "Security Manager OracleOnayTDHLogVW1 Recorder Init. " + er.Message);
            }
        }
        ...
        public bool Get_logDir()
        {
            ...
            try
            {
                ...
                err_log = rk.OpenSubKey("Remote Recorder").GetValue("Home Directory").ToString() + @"log\OracleOnayTDHLogVW1V_1_0_0Recorder" + Id + ".log";
                rk.Close();
                return true;
            }
            catch (Exception er)
            {
                EventLog.WriteEntry("Security Manager OracleOnayTDHLogVW1 Recorder Read Registry", er.ToString(), EventLogEntryType.Error);
                L.Log(LogType.FILE, LogLevel.ERROR, "Security Manager OracleOnayTDHLogVW1 Recorder Read Registry Error. " + er.Message);
                return false;
            }
            finally
            {
        ...
        public bool Read_Registry()
        {
            ...
            {
                rk = Registry.LocalMachine.OpenSubKey("SOFTWARE").OpenSubKey("Natek").OpenSubKey("Security Manager");
                log_size = Convert.ToUInt32(rk.OpenSubKey("Recorder").OpenSubKey("OracleOnayTDHLogVW1").GetValue("Log Size"));
                logging_interval = Convert.ToUInt32(rk.OpenSubKey("Recorder").OpenSubKey("OracleOnayTDHLogVW1").GetValue("Logging Interval"));
                trc_level = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("OracleOnayTDHLogVW1").GetValue("Trace Level"));
                err_log = rk.OpenSubKey("Agent").GetValue("Home Directory").ToString() + @"log\OracleOnayTDHLogVW1.log";
                db_name = rk.OpenSubKey("Recorder").OpenSubKey("OracleOnayTDHLogVW1").GetValue("DBName").ToString();
                this.timer1.Interval = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("OracleOnayTDHLogVW1").GetValue("Interval"));
                wsdb_name = rk.OpenSubKey("Recorder").OpenSubKey("OracleOnayTDHLogVW1").GetValue("WSDBName").ToString();
                max_record_send = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("OracleOnayTDHLogVW1").GetValue("MaxRecordSend"));
                last_recordnum = Convert.ToInt64(rk.OpenSubKey("Recorder").OpenSubKey("OracleOnayTDHLogVW1").GetValue("LastRecordNum"));
        ...
        private void timer1_Tick(object sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    rec.LogName = "OracleOnayTDHLogVW1V_1_0_0Recorder";
                    ...
                    rec.UserName = Convert_To_String(readReader["ADI_SOYADI"]);
                    rec.EventCategory = Convert_To_String(readReader["YETKI"]);
        ...
        public bool Set_Registry(long status)
        {
            ...
            try
            {
                rk = Registry.LocalMachine.CreateSubKey("SOFTWARE").CreateSubKey("Natek").CreateSubKey("Security Manager").CreateSubKey("Recorder").CreateSubKey("OracleOnayTDHLogVW1");
