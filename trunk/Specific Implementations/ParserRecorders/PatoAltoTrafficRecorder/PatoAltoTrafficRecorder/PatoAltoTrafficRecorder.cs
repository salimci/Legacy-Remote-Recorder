...
namespace Parser
{
    public class PaloAltoTrafficRecorder : Parser
    {
        public PaloAltoTrafficRecorder()
            : base()
        {
            LogName = "PaloAltoTrafficRecorder";
            usingKeywords = false;
            lineLimit = 50;
        }
        ...
        public PaloAltoTrafficRecorder(String fileName)
            : base(fileName)
        {
        }
        ...
        protected override void ParseFileNameLocal()
        {
            ...
            try
            {
                if (Dir.EndsWith("/") || Dir.EndsWith("\\"))
                {
                    ...
                    //PA-2020_traffic_2011_01_26_last_calendar_day.csv
                    foreach (String file in Directory.GetFiles(Dir))
                    {
                        string sFile = Path.GetFileName(file).ToString();
                        if (sFile.Contains("traffic") == true)
                        {
                            arrFileNames.Add(sFile);
                            Log.Log(LogType.FILE, LogLevel.DEBUG, " ParseFileNameLocal() | Got File Name : " + sFile);
                        }
                    }
        ...
        private string[] SortFiles(ArrayList arrFileNames)
        {
            ...
            try
            {
                for (int i = 0; i < arrFileNames.Count; i++)
                {
                    ...
                    {
                        Log.Log(LogType.FILE, LogLevel.DEBUG, "SortFiles() -->> Formata uymayan dosya : " + arrFileNames[i].ToString());
