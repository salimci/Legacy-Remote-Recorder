...
namespace MicrosoftForefront2010FwDBRecorder
{
    public class MicrosoftForefront2010FwDBRecorder : CustomBase
    {
        ...
        private string err_log, ffdb_namepart1 = "ISALOG", ffdb_namepart2 = "FWS", db_name = "",ffdb_name_last = "", forefrontDb_name = "", forefrontTable_name = "", location, user, password, remote_host = "";
            ...
            else
            {
                if (!reg_flag)
                {
                    ...
                    if (Database.AddConnection(db_name, Database.Provider.SQLServer, remote_host, user, password, "master"))
                        L.Log(LogType.FILE, LogLevel.INFORM, "  Successfully create MicrosoftForefront2010FwDBRecorder DAL");
                    else
                        L.Log(LogType.FILE, LogLevel.INFORM, "  Failed on creating MicrosoftForefront2010FwDBRecorder DAL");
                }   
            }
        }
        ...
        public bool get_logDir()
        {
            ...
            try
            {
                ...
                err_log = rk.OpenSubKey("Remote Recorder").GetValue("Home Directory").ToString() + @"log\MicrosoftForefront2010FwDBRecorder" + identity + ".log";
                return true;
            }
            catch (Exception ex)
            {
        ...
        public MicrosoftForefront2010FwDBRecorder()
        {
        ...
        public bool Read_Registry()
        {
            ...
            {   
                rk = Registry.LocalMachine.OpenSubKey("SOFTWARE").OpenSubKey("Natek").OpenSubKey("Security Manager");
                log_size = Convert.ToUInt32(rk.OpenSubKey("Recorder").OpenSubKey("MicrosoftForefront2010FwDBRecorder").GetValue("Log Size"));
                logging_interval = Convert.ToUInt32(rk.OpenSubKey("Recorder").OpenSubKey("MicrosoftForefront2010FwDBRecorder").GetValue("Logging Interval"));
                trc_level = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("MicrosoftForefront2010FwDBRecorder").GetValue("Trace Level"));
                err_log = rk.OpenSubKey("Agent").GetValue("Home Directory").ToString() + @"log\MicrosoftForefront2010FwDBRecorder.log";
                forefrontDb_name= rk.OpenSubKey("Recorder").OpenSubKey("MicrosoftForefront2010FwDBRecorder").GetValue("DBName").ToString();
                forefrontTable_name = rk.OpenSubKey("Recorder").OpenSubKey("MicrosoftForefront2010FwDBRecorder").GetValue("DTNAME").ToString();
                this.timer1.Interval = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("MicrosoftForefront2010FwDBRecorder").GetValue("Interval"));
                max_record_send = Convert.ToInt32(rk.OpenSubKey("Recorder").OpenSubKey("MicrosoftForefront2010FwDBRecorder").GetValue("MaxRecordSend"));
                //en son hangi recordda kaldigimizi bulmak icin
                lastRecordDate = Convert.ToString(rk.OpenSubKey("Recorder").OpenSubKey("MicrosoftForefront2010FwDBRecorder").GetValue("LastRecordDate"));
                rk.Close();
                return true;
            }
            catch (Exception er)
        ...
        {    
            ...
            {    
                ...
                if (checkDatabaseExists(ffdb_name_last))
                {
                    ...
                    L.Log(LogType.FILE, LogLevel.DEBUG, "  In timer1_Tick() -->> lastRecordDate :" + lastRecordDate.ToString());
                    ...
                    * */
                    ...
                    readQuery2 += "[logTime],[servername],[Action],[protocol],[resultcode],SourceIP As Source,";
                    readQuery2 += "DestinationIP As Destination,[rule],[SourceNetwork],";
                    readQuery2 += "[DestinationNetwork],[bytessent],[ApplicationProtocol],[SourcePort],[DestinationPort],[Bidirectional],[bytesrecvd] ";
                        ...
                        while (readReader2.Read())
                        {
                            ...
                            if (repeat == 0)
                            {
                                ...
                                try
                                {
                                    ...
                                    {
                                        curDate = readReader2.GetDateTime(0).ToString("yyyy/MM/dd HH:mm:ss.fff");
                                ...
                                L.Log(LogType.FILE, LogLevel.DEBUG, "  In timer1_Tick() -->>curDate IS " + curDate);
                                ...
                                try
                                {
                                    ...
                                    {
                                        rec.CustomStr1 = readReader2.GetString(3);     //protocol
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                try
                                {
                                    ...
                                    {
                                        rec.CustomStr2 = readReader2.GetInt32(4).ToString();   //resultcode 
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                try
                                {
                                    ...
                                    {
                                        rec.CustomStr3 = ResolveIp(readReader2.GetValue(5).ToString());  //SourceIP
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                L.Log(LogType.FILE, LogLevel.DEBUG, "  In timer1_Tick() -->>SourceIP  IS " + rec.CustomStr3);

                                try
                                {
                                    if (!readReader2.IsDBNull(6))
                                    {
                                        rec.CustomStr4 = ResolveIp(readReader2.GetValue(6).ToString());  //DestinationIP 
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                try
                                {
                                    ...
                                    {
                                        rec.CustomStr5 = readReader2.GetString(7);  //rule  
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                try
                                {
                                    ...
                                    {
                                        rec.CustomStr6 = readReader2.GetString(8);  // SourceNetwork
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                L.Log(LogType.FILE, LogLevel.DEBUG, "  In timer1_Tick() -->>SourceNetwork  IS " + rec.CustomStr6);

                                try
                                {
                                    if (!readReader2.IsDBNull(9))
                                    {
                                        rec.CustomStr7 = readReader2.GetString(9);  // DestinationNetwork
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                L.Log(LogType.FILE, LogLevel.DEBUG, "  In timer1_Tick() -->>DestinationNetwork  IS " + rec.CustomStr7);

                                try
                                {
                                    if (!readReader2.IsDBNull(10))
                                    {
                                        rec.CustomStr8 = readReader2.GetInt64(10).ToString();  // bytessent
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                L.Log(LogType.FILE, LogLevel.DEBUG, "  In timer1_Tick() -->>bytessent  IS " + rec.CustomStr8);

                                try
                                {
                                    if (!readReader2.IsDBNull(11))
                                    {
                                        rec.CustomStr9 = readReader2.GetString(11);  // ApplicationProtocol
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                L.Log(LogType.FILE, LogLevel.DEBUG, "  In timer1_Tick() -->ApplicationProtocol  IS " + rec.CustomStr9);

                                try
                                {
                                    if (!readReader2.IsDBNull(12))
                                    {
                                        rec.CustomInt1 = readReader2.GetInt32(12);   // SourcePort      
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                L.Log(LogType.FILE, LogLevel.DEBUG, "  In timer1_Tick() --> SourcePort   IS " + rec.CustomInt1.ToString());

                                try
                                {
                                    if (!readReader2.IsDBNull(13))
                                    {
                                        rec.CustomInt2 = readReader2.GetInt32(13);  // DestinationPort
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                L.Log(LogType.FILE, LogLevel.DEBUG, "  In timer1_Tick() --> DestinationPort  IS " + rec.CustomInt2.ToString());

                                try
                                {
                                    if (!readReader2.IsDBNull(14))
                                    {
                                        rec.CustomInt3 = Convert.ToInt32(readReader2.GetInt16(14));  // Bidirectional
                                    }
                                }
                                catch (Exception ex)
                                {
                                ...
                                L.Log(LogType.FILE, LogLevel.DEBUG, "  In timer1_Tick() -->Bidirectional  IS " + rec.CustomInt3.ToString());

                                try
                                {
                                    if (!readReader2.IsDBNull(15))
                                    {
                                        rec.CustomInt6 = readReader2.GetInt64(15);  // bytesrecvd
                                    }
                                }
                                catch (Exception ex)
                                {
