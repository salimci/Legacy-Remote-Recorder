...
namespace CiscoDhcpBindV_1_0_0Recorder
{
    public class CiscoDhcpBindV_1_0_0Recorder : CustomBase
    {
        ...
        public CiscoDhcpBindV_1_0_0Recorder()
        {
            InitializeComponent();
        }
        ...
        public override void Init()
        {
                ...
                if (usingRegistry)
                {
                    if (!reg_flag)
                    {
                        if (!Read_Registry())
                        {
                            ...
                            {
                                L.Log(LogType.FILE, LogLevel.ERROR, "Error on Intialize Logger on CiscoDhcpBindV_1_0_0Recorder functions may not be running");
                                return;
                            }
                        reg_flag = true;
                    }
            ...
            {
                EventLog.WriteEntry("Security Manager CiscoDhcpBindV_1_0_0Recorder Init", er.ToString(), EventLogEntryType.Error);
            }
        ...
        public bool Get_logDir()
        {
            ...
            try
            {
                ...
                err_log = rk.OpenSubKey("Remote Recorder").GetValue("Home Directory").ToString() + @"log\CiscoDhcpBindV_1_0_0Recorder" + ID + ".log";
                rk.Close();
                return true;
            }
            catch (Exception er)
            {
                EventLog.WriteEntry("Security Manager CiscoDhcpBindV_1_0_0Recorder Read Registry", er.ToString(), EventLogEntryType.Error);
                return false;
            }
            finally
            {
        ...
        public bool Set_Registry(long status)
        {
            ...
            try
            {
                rk = Registry.LocalMachine.CreateSubKey("SOFTWARE").CreateSubKey("Natek").CreateSubKey("Security Manager").CreateSubKey("Recorder").CreateSubKey("CiscoDhcpBindV_1_0_0Recorder");
